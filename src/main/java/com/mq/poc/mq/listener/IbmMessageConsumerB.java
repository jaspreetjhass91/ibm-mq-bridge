...
@Slf4j
@Component
public class IbmMessageConsumerB extends MqListener{

	@Autowired
	private RabbitTemplate rabbitTemplate;

	@Transactional(transactionManager = AppConstant.TRANSACTION_MANAGER)
	@JmsListener(destination = AppConstant.QUEUEB, containerFactory = AppConstant.CONNECTION_FACTORY_B, concurrency = "${ibm.mq.queue-a.concurrency}")
	...
	void publish(CustomTextMessage customTextMessage) {
		...
		log.info("message : {} sent successfully to queue B : {} ", customTextMessage);
	}
}
